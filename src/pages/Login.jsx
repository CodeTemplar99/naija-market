    ...
    <div className="auth-page">
      <div className="auth-left">
        ...
        <div className="auth-hero">
          <h1>Predict the<br /><span className="grad-text">Future.</span></h1>
          <p>Trade on real-world events in Nigeria. Politics, sports, crypto — earn from what you know.</p>
          <div className="auth-stats-mini">
            <div><span>₦485M+</span><small>Volume</small></div>
            <div><span>68K+</span><small>Traders</small></div>
            <div><span>200+</span><small>Markets</small></div>
          </div>
        </div>
      </div>
      ...
      <div className="auth-right">
        <div className="auth-card">
          <h2>Welcome back</h2>
          <p className="auth-sub">Sign in to your NaijaPredict account</p>
          ...
          <form className="auth-form" onSubmit={e => e.preventDefault()}>
            <div className="af-group">
              ...
              <div className="af-input-wrap">
                ...
                <input type="email" className="input" placeholder="you@example.com" value={form.email} onChange={e => u('email', e.target.value)} />
              </div>
            </div>
            <div className="af-group">
              <div className="af-label-row"><label>Password</label><a href="#" className="af-forgot">Forgot password?</a></div>
              <div className="af-input-wrap">
                <Lock size={16} className="af-icon" />
                <input type={show ? 'text' : 'password'} className="input" placeholder="Enter password" value={form.password} onChange={e => u('password', e.target.value)} />
                <button type="button" className="af-eye" onClick={() => setShow(!show)}>{show ? <EyeOff size={16} /> : <Eye size={16} />}</button>
              </div>
            </div>
            <Link to="/" className="btn btn-primary btn-lg w-full">Sign In <ArrowRight size={18} /></Link>
          </form>
      ...
      <style>{`
        ...
        .auth-stats-mini { display: flex; gap: 2.5rem; }
        .auth-stats-mini div { display: flex; flex-direction: column; }
        .auth-stats-mini span { font-size: 1.5rem; font-weight: 800; color: white; font-family: var(--font-display); }
        .auth-stats-mini small { font-size: 0.7rem; color: rgba(255,255,255,0.5); font-weight: 700; text-transform: uppercase; }

        .auth-right { width: 520px; display: flex; align-items: center; justify-content: center; padding: 2rem; background: var(--bg); }
        ...
        .af-label-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.4rem; }
        .af-forgot { font-size: 0.75rem; color: var(--primary-light); font-weight: 700; }
