...
import { ChevronLeft, Info, TrendingUp, Users, ShieldCheck, Share2, MessageSquare, Clock, ExternalLink, ThumbsUp, ThumbsDown } from 'lucide-react'
  ...
  return (
    <div className="detail-page animate-slide-up">
      ...
      <div className="dt-grid">
        ...
        <div className="dt-main">
          ...
          <div className="outcome-bar glass">
            <div className="ob-side yes">
              <span className="ob-label">Yes</span>
              <span className="ob-pct">{market.yesPrice}%</span>
            </div>
            <div className="poll-bar-track big">
              <div className="poll-bar-fill yes" style={{ width: `${market.yesPrice}%` }}></div>
            </div>
            <div className="ob-side no">
              <span className="ob-label">No</span>
              <span className="ob-pct">{market.noPrice}%</span>
            </div>
          </div>
              ...
              <svg viewBox="0 0 600 200" className="chart-svg">
                <defs>
                  <linearGradient id="chartGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="var(--accent-yes)" stopOpacity="0.3" />
                    <stop offset="100%" stopColor="var(--accent-yes)" stopOpacity="0" />
                  </linearGradient>
                </defs>
                <path d="M0,160 C50,150 100,140 150,120 C200,100 250,110 300,80 C350,50 400,70 450,60 C500,50 550,40 600,30"
                  fill="none" stroke="var(--accent-yes)" strokeWidth="2.5" />
                <path d="M0,160 C50,150 100,140 150,120 C200,100 250,110 300,80 C350,50 400,70 450,60 C500,50 550,40 600,30 L600,200 L0,200 Z"
                  fill="url(#chartGrad)" />
              </svg>
          ...
          <div className="about-box glass">
            ...
            <ul className="rules">
              {market.rules.map((r, i) => (
                <li key={i}><ShieldCheck size={15} color="var(--primary-light)" /> {r}</li>
              ))}
            </ul>
          </div>
          ...
          <div className="comments-box glass">
            <div className="cm-header">
              ...
              <h3>Discussion ({market.comments})</h3>
            </div>
                  ...
                  <Link to={`/market/${r.id}`} key={r.id} className="related-card glass">
                    ...
                    <div className="rel-info">
                      ...
                      <div className="rel-prices">
                        <span className="rel-yes">Yes {r.yesPrice}%</span>
                        <span className="rel-no">No {r.noPrice}%</span>
                      </div>
                    </div>
                  </Link>
                ))}
        ...
        <aside className="dt-sidebar">
          <div className="trade-card glass">
            <div className="tc-tabs">
              <button className={`tc-tab ${selectedOutcome === 'Yes' ? 'yes active' : ''}`} onClick={() => setSelectedOutcome('Yes')}>
                Buy Yes
              </button>
              <button className={`tc-tab ${selectedOutcome === 'No' ? 'no active' : ''}`} onClick={() => setSelectedOutcome('No')}>
                Buy No
              </button>
            </div>
            ...
            <div className="tc-body">
              ...
              <div className="tc-summary">
                ...
                <div className="tc-row highlight">
                  <span>Potential Return</span>
                  <span className="tc-win">₦{Number(potentialWin).toLocaleString()} <small>(+{returnPct}%)</small></span>
                </div>
              </div>

              <button className={`btn w-full ${selectedOutcome === 'Yes' ? 'btn-yes-full' : 'btn-no-full'}`}>
                Buy {selectedOutcome}
              </button>

              <p className="tc-disclaimer"><Info size={11} /> Settled instantly upon resolution. 7-day refund policy.</p>
            </div>
          </div>
          ...
          <div className="ob-card glass">
            ...
            <div className="ob-grid">
              <div className="ob-col">
                <p className="ob-head yes-c">Bids (Yes)</p>
                <div className="ob-r"><span>₦{market.yesPrice - 0.5}</span><span>2.4M</span></div>
                <div className="ob-r"><span>₦{market.yesPrice - 1}</span><span>800k</span></div>
                <div className="ob-r"><span>₦{market.yesPrice - 2}</span><span>1.5M</span></div>
              </div>
              <div className="ob-col">
                <p className="ob-head no-c">Asks (No)</p>
                <div className="ob-r"><span>₦{market.noPrice - 0.5}</span><span>1.2M</span></div>
                <div className="ob-r"><span>₦{market.noPrice - 1}</span><span>500k</span></div>
                <div className="ob-r"><span>₦{market.noPrice - 2}</span><span>3.1M</span></div>
              </div>
            </div>
          </div>
        </aside>
      ...
      <style>{`
        ...
        .ct { background: none; border: none; color: var(--text-muted); padding: 0.3rem 0.75rem; border-radius: 6px; font-weight: 700; font-size: 0.75rem; cursor: pointer; }
        ...
        .cm-action { background: none; border: none; color: var(--text-muted); font-size: 0.75rem; cursor: pointer; display: flex; align-items: center; gap: 0.3rem; font-weight: 600; }
        .cm-action:hover { color: var(--text); }
        ...
        .tc-quick button:hover { background: var(--surface-light); }
